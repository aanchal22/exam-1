...
class Tests:
  ...
  def test_replace_email_domain_shape(self, old_domain, new_domain):
    ...
    assert len(actual_users_data_replaced_email) == expected_users_df_non_null.shape[0]
    assert all([len(x) == expected_users_df_non_null.shape[1] for x in actual_users_data_replaced_email])
  ...
  def test_get_average_and_median_affinity_id(self, affinity_type):
    ...
    assert abs(returned_average - expected_affinity_average) < 10**-2
